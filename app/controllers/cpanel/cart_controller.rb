class Cpanel::CartController < CpanelController
  ...
  def index
    @market_order_market_products = current_merchant.market_order_market_products.where(:market_order_id => nil).order("created_at DESC").paginate(:page => params[:page], :per_page => 10)
  end
